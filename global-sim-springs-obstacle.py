#%%
# Obstacle detection with an always-on spring network: takes global-sim-person.py's
# fast/vectorized simulation loop, moving-obstacle sensing (local_repulsion), and full
# analysis pipeline wholesale, but replaces its spring construction with
# global-simulation-simplified.py's - every robot pair connected by one shared,
# symmetric spring (K[i,j] == K[j,i], one random draw per pair, matching
# spring_k()), rather than global-sim-person.py's independently-asymmetric-per-cell K
# plus an Erdos-Renyi CONNECTIVITY_PROB sparsity mask.
#
# Why this exists rather than just tuning CONNECTIVITY_PROB up in global-sim-person.py:
# that file's currently-tuned SETUP has CONNECTIVITY_PROB=0, i.e. the spring network
# contributes exactly zero force right now (K is masked to all-zeros) - the "best
# trial" numbers there were found with springs switched off entirely. This file starts
# from a genuinely fully-connected, always-on spring network instead, to see how the
# swarm reacts to a moving obstacle when robots are actually coupled to each other via
# springs (not just Du's algebra sitting inert). BETA_SCALE/K_SCALE/K_SELF_SCALE/
# R_GLOBAL_FRAC/K_GLOBAL_SCALE below are carried over from global-sim-person.py's
# tuned values as a starting point only - they were never tuned WITH the spring network
# actually active, so treat them as a first guess, not a calibrated result.
#
# Everything else (Du's spring formula, local_repulsion, _step_core, step, DT/
# num_iterations scale, and the whole analysis pipeline at the bottom) is unchanged
# from global-sim-person.py.
...
def Du(x, y, x0, y0, K, A, K_self):
    # K[i, j] is the stiffness robot i perceives towards robot j - here
    # always symmetric (K[i,j] == K[j,i], one shared draw per pair - see
    # SETUP) and fully connected, unlike global-sim-person.py's
    # independently-asymmetric-per-cell K plus connectivity sparsity mask.
    N = x.shape[0]
    sum_x = np.zeros(N)
    sum_y = np.zeros(N)
    for i in range(N):
...
# starting point only - see module docstring: these are global-sim-person.py's
# tuned values, carried over as a first guess, but that search ran with
# CONNECTIVITY_PROB=0 (springs off) - it never actually tuned against a live
# spring network, so don't trust these as calibrated for this file.
...
K_BASE, K_SPAN = 7.8788, 5.0  # matches deterministic_k()'s draw range (global-simulation.py)


def spring_k():
    # global-simulation-simplified.py's spring_k(): one draw per unordered
    # pair, shared by both endpoints - K[i,j] == K[j,i], unlike
    # global-sim-person.py's independent-per-cell (asymmetric) draw.
    return (K_BASE + np.random.uniform() * K_SPAN) * K_SCALE


# symmetric, fully-connected spring matrix - one draw per unordered pair
# (i, j), no connectivity sparsity mask (every pair is always a spring, K=0
# only on the diagonal). See module docstring for why this differs from
# global-sim-person.py's K construction.
K = np.zeros((N, N))
for i in range(N):
